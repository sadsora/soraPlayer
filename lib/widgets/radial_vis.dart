...
class Radial extends CustomPainter {
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    ...
    final bandsNum = bands.length - 28;
    ...
    final perAngle = 2 * pi / (3 * bandsNum);
    ...
    final brightLine = Path();

    brightLine.moveTo(center.dx + innerRadius, center.dy);

    for (int i = 0; i < bandsNum; i++) {
      final value = bands[i].clamp(0.0, 1.0);
      final rawAmp = outerRadius * ((value < 0.4) ? value / 4 : value / 3);
      for (int j = 0; j < 3; j++) {
        final a = i * (j + 1) * perAngle;
        //final a = i * perAngle + j * (perAngle / 3);
        brightLine.moveTo(
    ...
    brightLine.close();
    canvas.drawPath(brightLine, brightPaint);
  ...
  @override
  bool shouldRepaint(covariant Radial old) =>
      old.spec != spec ||
